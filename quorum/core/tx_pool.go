...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	log.Info("=====> Validating Transaction", "size", tx.Size())
	...
	log.Info("=====> Passed transaction size limit test")
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (bool, error) {
	...
	if err := pool.validateTx(tx, local); err != nil {
		log.Info("=====> Error in validating transaction", "err", err, "hash", hash)
		log.Trace("Discarding invalid transaction", "hash", hash, "err", err)
		invalidTxCounter.Inc(1)
		return false, err
	}

	log.Info("=====> Transaction validation returned with no errors")
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) {
	...
	for _, addr := range accounts {
		...
		for _, tx := range list.Ready(pool.pendingState.GetNonce(addr)) {
			...
			log.Info("====> Promoting executable transaction", "hash", hash)
